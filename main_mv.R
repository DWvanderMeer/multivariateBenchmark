...
for(stn in 1:length(station)){ # Loop over the stations
  ...
  ICS1 <- data.table::shift(surfrad$Ineichen,n=K,type="lead"); ICS1 <- do.call(cbind,ICS1) # Create vector clear-sky irradiance
  ICS2 <- data.table::shift(surfrad$McClear,n=K,type="lead"); ICS2 <- do.call(cbind,ICS2) # Create vector clear-sky irradiance
  ...
  for(j in 1:length(yrs)){
    ...
    idx_zen <- zen <= zen_angle # Create zenith mask
    CSI1[idx_zen == FALSE] <- NA; CSI2[idx_zen == FALSE] <- NA; CSI3[idx_zen == FALSE] <- NA
...
res_3 <- do.call(rbind,rank_histograms3)
...
res_4 <- do.call(rbind,num_score_1)
...
res_5 <- do.call(rbind,num_score_2)
...
res_6 <- do.call(rbind,num_score_3)
