...
for(stn in 1:length(station)){ # Loop over the stations
  ...
  for(j in 1:length(yrs)){
    ...
    CSI1 <- ifelse(CSI1 > 1.5, 1.5, CSI1); CSI1 <- ifelse(CSI2 > 1.5, 1.5, CSI2) # Limit the CSI values to max = 1.5
    ...
    for(t in 1:nrow(OBS1)){ # Loop over the test set
      ...
      ci1 <- ci1$ICS1 # Take only the clear-sky irradiance vector
      ci2 <- ci2$ICS2 # Take only the clear-sky irradiance vector
      fc1 <- fc1$CSI1 # Take only teh forecast matrix
      fc2 <- fc2$CSI2 # Take only teh forecast matrix
      ...
      crps1[[i]] <- scoringRules::crps_sample(y=ob,dat=fc1)
      crps2[[i]] <- scoringRules::crps_sample(y=ob,dat=fc2)
      
      # Compute PIT:
      fn1 <- ecdf(fc1); fn2 <- ecdf(fc2)
      pit1[[i]] <- fn1(ob); pit2[[i]] <- fn2(ob)
      i <- i + 1
    }
    # PIT histograms:
    pit_hist1 <- do.call(c,pit1)
    pit_hist1 <- data.frame(pit=pit_hist1,years=paste(yrs[[j]][2],"-",yrs[[j]][1],sep=""),
                           station=toupper(station[stn]),model="Ineichen")
    pit_hist2 <- do.call(c,pit2)
    pit_hist2 <- data.frame(pit=pit_hist2,years=paste(yrs[[j]][2],"-",yrs[[j]][1],sep=""),
